...
function drawTimer() {
    ...
    $(timer.selector).css('stroke-dashoffset', '440' - (timer.i * ('440' / timer.time)));
    ...
    $('h2.section').html("Current Section: " + (currentSection + 1) + " / " + sections.length + "   <button class=\"button\" onclick=\"skipSect();\">Next</button>");
    $("div.sections").html(sectionvis);
    ...
    if (timer.i == timer.time) {
        ...
        timer.i = 1;
        currentSection++;
        timer.time = sections[currentSection] * 60;
    }

    if (isNaN(timer.time - timer.i)) {
...
for (let i = 0; i < sections.length; i++) {
    sections[i] = parseInt(prompt("Length (Minutes) of Section #" + (i + 1) + ":"));
...
var tim = new Timer(sections[currentSection], '.circle_animation');
...
var totaltim = new Timer(total, '.circle_animation_total');
